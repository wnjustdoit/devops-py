...
class PublishmentNodejs(Base, Entity):
    __tablename__ = 'publishment_nodejs'

    id = Column(Integer, Sequence('publishment_nodejs_id_seq'), primary_key=True, comment='主键id')
    ...
    git_repo = relationship("GitRepo", uselist=False, primaryjoin='PublishmentNodejs.git_repo_id == GitRepo.id',
                            lazy=False)
...
class PublishmentNodejsSchema(Schema):
