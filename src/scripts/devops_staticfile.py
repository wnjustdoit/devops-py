#!/usr/bin/env python3
# usage: python3 [script].py [--work_home=] [--git_repo=] [--git_branches=] [--project_name=] [--source_file_dir] [--to_ip=] [--to_project_home=]
# eg: python3 devops_staticfile.py --work_home=/tmp/devops --git_repo=git@192.168.1.248:mall/common.git --git_branches=master --project_name=docs --source_file_dir=assets --to_ip=192.168.1.248 --to_project_home=/home/project/mama_docs
...
def publish(git_repo, git_branch, project_name, source_file_dir, to_ip, to_project_home):
    # verify parameters
    if git_repo is None or git_repo.strip() == '' \
        ...
        output_error(
            ...
            f'project_name:{project_name}, to_ip:{to_ip}, to_project_home:{to_project_home}')
...
if __name__ == '__main__':
    # define variables
    git_repo = git_branches = project_name = source_file_dir = to_username = to_ip = to_project_home = None
    ...
    publish(git_repo, git_branches, project_name, source_file_dir, to_ip, to_project_home)
