#!/usr/bin/env python3
# usage: python3 [script].py  [--git_repo=] [--git_branches=] [--project_name=] [--from_file_path] [--to_username=] [--to_ip=] [--to_project_home=]
# eg: python3 devops_staticfile.py --git_repo=git@192.168.1.248:mall/common.git --git_branches=master --project_name=common --from_file_path=aaa --to_username=root --to_ip=192.168.1.248 --to_project_home=/home/project/mama_docs
...
if __name__ == '__main__':
    # define variables
    git_repo = git_branches = project_name = from_file_path = to_username = to_ip = to_project_home = None

    # receive parameters
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hgr:gb:pn:ffp:tu:ti:tph",
                                   ["help", "git_repo=", "git_branches=", "project_name=", "from_file_path=",
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            project_name = arg
        elif opt in ("--from_file_path", "-ffp"):
            from_file_path = arg
